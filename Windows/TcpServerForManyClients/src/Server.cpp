...
void TcpServer::HandleClients()
{
    ...
    while (true)
    {
        iRes = WSAPoll(m_fds.data(), m_fds.size(), 200);
        ...
        if (m_fds.at(0).revents & POLLIN) //if there are new client(s)
        {
            ...
            if (newClientPollFd.fd == INVALID_SOCKET)
            {
                std::cout << "error accepting new client: " << WSAGetLastError() << std::endl;
                continue;
            }
        ...
        {
            if (m_fds.at(i).revents & POLLHUP)
            {
                std::cout << "Client disconnected fd = [" << m_fds.at(i).fd << "]!" << std::endl;
                closesocket(m_fds.at(i).fd);
